    ...
    <div class="panel">
      <div style="position: relative; height: 100%">
        ...
        <a-row :gutter="24" style="margin-bottom: 12px">
          ...
          <a-col :span="6">
			 <a-select
			   v-model="crud.options.query.online"
			   placeholder="选择状态搜索"
			   allow-search>
			   <a-option
				 v-for="s in dict.online_status"
				 :key="s.detailId"
				 :value="s.value"
			   >
				 {{ s.label }}
       ...
       <!--查询表单-->
        <a-row :gutter="24" style="margin-bottom: 12px">
         ...
         <!--大分类搜索框-->
          <a-col :span="6">
            <a-select
              v-model="crud.options.query.channelId"
              placeholder="选择视频源搜索"
            >
              <a-option
                v-for="channel in channelMap"
                :key="channel.id"
                :value="channel.id"
              >
                {{ channel.name }}
          ...
          <a-col :span="6">
            <a-select
              v-model="crud.options.query.kindId"
              placeholder="选择大分类搜索"
            >
              <a-option
                v-for="kind in kindMap"
                :key="kind.id"
                :value="kind.id"
              >
                {{ kind.name }}
          ...
          <a-col :span="6">
            <a-select
              v-model="crud.options.query.categoryId"
              placeholder="选择小分类搜索"
            >
              <a-option
        ...
        >
		 ...
		 <template #online="{ record }">
		   <!--正常情况下-->
		   <div v-show="!record.editable && !crud.options.tableInfo.isEdit">
			 {{ dict.online_status ? (dict.online_status.filter((di: any) => di.value === (record.online ? '1' : '0') || di.value === (record.online + '')))[0].label : ''}}
		   </div>
		 ...
		 <template #operate="{ record }">
		   ...
		   <a-button
		     v-permission="['video:live:pull']"
		     :loading="loading"
			 
		     type="outline"
		     size="mini"
		     status="danger"
		     @click="rePull(record.id)"
		   >
		     <template #icon>
		       <icon-forward />
		     </template>
...
<script lang="ts" setup>
  ...
  import { Live } from '@/api/video/live';
  ...
  // 字典
  const dict = useDict('online_status');
  ...
  const rePull = async (liveId: number) => {
    toggle();
    const res = await axios.post('/video/live/pull', {id: liveId});
    try {
  ...
  crud.update.setTableColumns([
    ...
    {
      title: '最后一次拉取时间',
      dataIndex: 'lastPullTime',
      width: 180,
      display: false,
      slotName: 'lastPullTime',
      tooltip: true,
      ellipsis: true,
    },
	{
  ...
  // 获取服务器和应用信息
  const getKind = async () => {
    const kindList = await KindList("1");
    kindMap.value = {};
    kindList.data.list.forEach((val) => {
      kindMap.value[val.id] = val;
    });
	
	const categoryList = await CategoryList("1");
	categoryMap.value = {};
	...
	const channelList = await ChannelList();
	channelMap.value = {};
